...
<template>
    <div v-if="!loading" class="book-grid">
        <div class="w-[200px] h-[260px] border border-transparent cursor-pointer hover:border-gray-700 rounded-md group" 
            v-for="book in books" :key="book.id"
            @click="viewBook(book.id)">
            <div class="flex flex-col p-5 overflow-hidden">
                <img 
                    class="w-[150px] h-[200px] group-hover:opacity-10 transition
    ...
    <div v-else="loading" class="book-grid">
        <div class="w-[200px] h-[260px] flex flex-col p-5" v-for="index in 20" :key="index">
            <div class="w-[150px] h-[200px] placeholder"></div>
        </div>
    </div>
</template>